                ...
                var Main = (function (_super) {
                    ...
                    function Main() {
                        ...
                        this.sqlSynchronized = function (ids) {
                            var sql = 'SELECT p.id, p.description, p.involved, p.suggestion, p.actiontaken, it.name as incidencetypename, p.date, p.time, p.geometry, p.uuid, p.spatialreferenceid, p.documenturl, p.registrationdate, p.state FROM "inc.incidence" as p inner join "inc.incidencetype" as it on p.incidencetypeid = it.id where p.id in(' + ids.join() + ') order by p.registrationdate desc';
                            return sql;
                        }
                                ...
                                for (i = 0; i < length; i++) {
                                    (function (k) {
                                        var data = {
                                            ...
                                            involved: rs.rows.item(k).involved,
                                            suggestion: rs.rows.item(k).suggestion,
                                            actiontaken: rs.rows.item(k).actiontaken,
                                            incidence: { name: rs.rows.item(k).incidencetypename },
                                            date: rs.rows.item(k).date,
                                            time:rs.rows.item(k).time,
                                            ...
                                            fileEntry.file(function (file) {
                                                ...
                                                reader.onloadend = function (e) {
                                                    ...
                                                    thiss.ajax({
                                                        ...
                                                        url: thiss.config.restServer + "incidence/incidence/save",
                                                        async: true,
                                                        validate: function () { return true; }
                                                    }, function (data) {
                                                        j++;
                                                        thiss.db.executeSql("update 'inc.incidence set issynchronized=? where uuid=?", [true, data.extra]);
                                                        if (j === length) {
                        ...
                        confirm("¿Desea eliminar el elemento?", function (buttonId) {
                            if (buttonId === 1) {
                                ...
                                thiss.db.transaction(function (tx) {
                                    tx.executeSql('delete from "inc.incidence" where id in (' + ids.join() + ')', []);
                                }, function (error) {
                    ...
                    Main.prototype.search = function (url, options, callback) {
                        ...
                        this.db.readTransaction(function (tx) {
                            tx.executeSql('SELECT * FROM "inc.incidence" where lower(description) LIKE  "%' + thiss.$txtDescription.string().toLowerCase() + '%" order by registrationdate desc', [], function (tx, rs) {
                                var length = rs.rows.length;
                    ...
                    Main.prototype.load = function () {
                        _super.prototype.load.call(this, "", { module: "incidence", subModule: 'incidence' }); ///
                    };
...
com.jtm.incidence.incidence.Main.execute();