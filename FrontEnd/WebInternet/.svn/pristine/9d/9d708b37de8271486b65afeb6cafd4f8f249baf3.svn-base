function getInstance() {
    return com.jtm.miningconcession.miningrequest.Form.execute();
}
(function (com) {
    (function (jtm) {
            ...
            (function (miningrequest) {
                var Form = (function (_super) {
                    ...
                    Form.prototype.domConfig = function () {
                        ...
                        this.$divOption = this.$form.find('#divOption');
                        this.$tableCoordinates = this.$form.find('#tblResultCoordinates');
                    };
                    ...
                    Form.prototype.save = function (url, data, options, callback) {
                        ...
                        prompt("Ingrese el nombre", function (result) {
                            ...
                            thiss.db.transaction(function (tx) {
                                if (data.id === 0)
                                    tx.executeSql('INSERT INTO "mc.miningrequest"(name,uuid,geometry,spatialreferenceid,registrationdate,state,issynchronized) VALUES (?,?,?,?,?,?,?)',
                                    ...
                                    tx.executeSql('UPDATE "mc.miningrequest" SET name=?,geometry=?,spatialreferenceid=?,registrationdate=?, state=? where id=?', [thiss.$txtName, geometry, data.spatialReference.id, Date.now(), true, data.id]);
                            }, function (error) {
                    ...
                    Form.prototype.detail = function (url, id, options) {
                        ...
                        thiss.db.executeSql('SELECT mr.* FROM "mc.miningrequest" as mr where mr.id=?', [id], function (rs) {
                    ...
                    Form.prototype.load = function (id) {
                        var thiss = this;
                        _super.prototype.load.call(this, com.jtm.Server.contextPath + "areas/miningconcession/miningrequest/_form.html", id, {
